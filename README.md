...
Lo script accede a Google Sheets tramite OAuth 2.0 con flusso desktop. Le credenziali sono condivise con tutti gli altri tool llmwiki e vanno collocate in:
...
5. Scarica il file JSON e salvalo nel percorso sopra (crea la directory se non esiste)
6. Se l'app è in modalità test, aggiungi il tuo account Google come *Test user* in *OAuth consent screen*

## Utilizzo

```bash
uv run main.py "<URL_FOGLIO>" [--sheet 0]
```
...
Al primo avvio si apre il browser per autorizzare l'accesso. Dopo il consenso, il token viene salvato in `~/.config/llmwiki/obs-llmwiki-simone-personal-v1/token-sheets.json` e riutilizzato automaticamente nelle esecuzioni successive (con refresh automatico alla scadenza).
...
Credenziali e token OAuth risiedono in `~/.config/llmwiki/obs-llmwiki-simone-personal-v1/` e non fanno parte di questo repository. Non commitare quei file.
